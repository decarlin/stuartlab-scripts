...
sub main() {					# Main program
  ...
  my $edgeWeightCol = undef;
  my $edgeColorCol = undef;
  my $edgeLabelCol = undef;

  my $nodeFillColorCol = undef;
  my $nodeOutlineColorCol = undef;
  my $nodeShapeCol = undef;
  my $nodeIdCol = undef;
  my $nodeLabelCol = undef;
  my $nodeXPosCol = undef;
  my $nodeYPosCol= undef;
  ...
  GetOptions("help|?|man" => sub { printUsage(); }
			 ...
			 , "eweight=i"   => \$edgeWeightCol    # the column (numbered from *1* where the edge weight is (in the edges file)
			 , "ecolor=i"   => \$edgeColorCol      # the column (numbered from *1* where the edge color is (in the edges file)
			 , "elabel=i"   => \$edgeLabelCol      # the column (numbered from *1* where the edge label is (in the edges file)
			) or printUsage();
  ...
  print $OPEN; {
	...
	open(NODES, "< $nodeFile") or die "Could not open NODES file $nodeFile.\n"; {
	  ...
	  while (my $line = <NODES>) {
		...
		my $nodeLabel = readColIfExistsElseDefault(\@a, $nodeLabelCol, $a[0]);
		my $nodeId = readColIfExistsElseDefault(\@a, $nodeIdCol, undef);
		...
		my $nodeXPosition = readColIfExistsElseDefault(\@a, $nodeXPosCol, 100);
		my $nodeYPosition = readColIfExistsElseDefault(\@a, $nodeYPosCol, 100);
		...
		if (!defined($nodes{$nodeLabel})) {
		  if(!defined($nodeId)) {
		    if(!defined($nodeIdCol)) {
		      ...
		      die("Failed to get identifier for '$nodeLabel' ($numNodesRead nodes read).");
		    }
		  }
		  $nodes{$nodeLabel} = $nodeId;
		  $nodeIds{$nodeId} = 1;
		  $numNodesRead++;
		}
		...
		if (!defined($nodesAlreadyPrinted{$nodeLabel})) {
		  ...
		  print $OPEN; {
			...
			print "graphics\n";
			print $OPEN; {
			  print "x" . "\t" . $nodeXPosition . "\n";
			  print "y" . "\t" . $nodeYPosition . "\n";
			  print "w" . "\t" . $nodeWidth . "\n";
			  print "h" . "\t" . $nodeHeight . "\n";
	...
	open(EDGES, "< $edgeFile") or die "Could not open  file $edgeFile.\n"; {
	  ...
	  while (my $line = <EDGES>) {
		...
		my $edgeSource = $e[0];
		my $edgeTarget = $e[1];
		my $edgeWeight = readColIfExistsElseDefault(\@e, $edgeWeightCol, "1");
		my $edgeColor  = readColIfExistsElseDefault(\@e, $edgeColorCol, "#0000E1");
		my $edgeLabel  = readColIfExistsElseDefault(\@e, $edgeLabelCol, "pp");
		...
		if (!defined($nodes{$edgeSource})) {
		  if(!defined($nodeIdCol)) {
		    for($maxIntId = $maxIntId+1; defined($nodeIds{$maxIntId}); $maxIntId++) {
		    }
		    $nodes{$edgeSource} = $maxIntId;
		  }
		  elsif($nodeIdCol <= 0) {
		    $nodes{$edgeSource} = $edgeSource;
		  }
		  else {
		    die("Failed to get identifier for node $numNodesRead.");
		  }
		  $numNodesRead++;
		}
		if (!defined($nodes{$edgeTarget})) {
		  if(!defined($nodeIdCol)) {
		    ...
		    $nodes{$edgeTarget} = $maxIntId;
		  }
		  elsif($nodeIdCol <= 0) {
		    $nodes{$edgeTarget} = $edgeTarget;
		  }
		  else {
		    die("Failed to get identifier for node $numNodesRead.");
		  }
...
--ni = COLUMN_INDEX (counting from 1)
  Column in the nodes file containing the identifier for the node. If
  the index is <=0 then a node ID taken from the same value as the
  label. The default is to automatically generate an identifier as
  an integer. Note that Cytoscape requires identifiers to be integers.

--nx = COLUMN_INDEX (counting from 1)
  Column in the nodes file containing the X-coordinate. Default is 100.

--ny = COLUMN_INDEX (counting from 1)
  Column in the nodes file containing the Y-coordinate. Default is 100.
...
--eweight = COLUMN_INDEX (counting from 1)
  Column in the edges file where we find the edge weight. This can be
  any score (usually positive) for layout purposes.

--ecolor = COLUMN_INDEX (counting from 1)
  Column in the edges file where we find the edge line color. Colors
  are of the format #RRGGBB.

--elabel = COLUMN_INDEX (counting from 1)
  Column in the edges file where we find the edge label. By default, the
  label is 'pp' for protein-protein interaction in Cytoscape.
