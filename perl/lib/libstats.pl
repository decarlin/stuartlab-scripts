...
#---------------------------------------------------------
# vec_prod - Take the product of all of the components.
#---------------------------------------------------------
sub vec_prod
{
	my ($vec) = @_;
...
sub vec_stdmed
{
	...
	if($len > 0) {
           my $med = &vec_median($vec);
           my $sumsqr = &vec_length(&vec_add_scalar($vec, -$med));
	   $result = sqrt($sumsqr / $len);
	}
